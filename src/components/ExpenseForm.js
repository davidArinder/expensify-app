...
export default class ExpenseForm extends React.Component {
  ...
  onDateChange = (createdAt) => {
    if (createdAt) {
      // if there is a createdAt value run this.setState, else do nothing; prevents user from deleting the date
  ...
  onSubmit = (e) => {
    ...
    if (!this.state.description || !this.state.amount) {
      ...
      })); // if no description or amount, show error
    } else {
      this.setState(() => ({ error: "" })); // if description and amount present, remove error
      this.props.onSubmit({
        // this onSubmit prop created on AddExpensePage.js
        ...
        note: this.state.note ? this.state.note : "No note provided.",
      });
    }
  };
  render() {
